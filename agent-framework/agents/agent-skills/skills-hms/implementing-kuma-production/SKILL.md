...
**Template**:
```yaml
# Global CP deployment - policy configuration only
# Network isolation: Rejects direct DP connections
# Accepts connections from Zone CPs only
```
...
**Template**:
```yaml
# Zone CP deployment - local execution layer
# Accepts DP connections from local zone
# Receives policy updates from Global CP
# Computes XDS configuration for local DPs
```
...
spec:
  targetRef:
    kind: Mesh  # Coarse-grained target
  to:
    - targetRef:
        kind: Mesh
      default:
...
kind: ContainerPatch
metadata:
  name: production-sidecar-sizing
...
Zone Ingress proxies handle cross-zone traffic routing. Built-in DNS server resolves services to local replicas or remote Zone Ingress addresses, enabling automatic failover. mTLS is mandatory for cross-zone communication to establish identity context.
...
External API gateway (Kong) handles perimeter security (rate limiting, authentication, request timeouts). Kuma manages internal service-to-service traffic, applying L4/L7 policies. Clear separation of concerns: gateway for external, mesh for internal.
...
**Critical Sequence**:
1. Deploy permissive MeshTrafficPermission allowing all traffic
2. Enable mTLS on Mesh resource
...
**Configuration**: Use ContainerPatch to set minimum 100m CPU, 128Mi memory requests. Set high limits (1 CPU, 1GB) to prevent throttling.
...
**Resolution**: Enable mTLS on Mesh resource before configuring cross-zone routing. Ensure Zone Ingress proxies have valid certificates.
...
**Resolution**: Apply ContainerPatch CRD with production-ready resource requests (100m CPU, 128Mi memory minimum) and high limits.

**Detection**: Envoy throttling metrics, high latency, pod resource constraints, default resources in use.

## Code Templates

### Template 1: Mesh mTLS with CA Rotation
...
### Template 3: ContainerPatch for Sidecar Sizing
...
### Template 4: MeshTrafficPermission (Permissive Baseline)
...
### Template 5: Delegated Gateway Configuration
...
### Example 1: Multi-Zone E-Commerce Platform

**Scenario**: E-commerce platform spanning 3 zones (US, EU, Asia) requiring cross-region order processing and automatic failover.

**Implementation**: Global CP deployed in central region managing policy configuration. Zone CPs deployed per region managing local DPs. Zone Ingress proxies enable cross-region routing. Built-in DNS resolves orders service to local replica or remote Zone Ingress.

**Configuration**:
```yaml
# Zone CP in US region
apiVersion: kuma.io/v1alpha1
kind: Zone
metadata:
  name: us-east
...
### Example 2: Hybrid Kubernetes/VM Migration

**Scenario**: Legacy VM applications integrated with Kubernetes microservices under unified service mesh governance.

**Implementation**: Single Global CP manages both Kubernetes (sidecar-injected) and Universal (VM) data planes. VM workloads defined via Dataplane resources. kuma-dp process started on VMs via automation tools (Ansible/Terraform).

**Configuration**:
```yaml
# Universal Dataplane for VM workload
apiVersion: kuma.io/v1alpha1
kind: Dataplane
metadata:
  name: legacy-app-vm
...
- `templates/`: Full YAML policy templates
